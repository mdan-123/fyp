#!/usr/bin/env python3
"""
evaluate_checkpoints_neobert.py
================================
Evaluates every checkpoint (plus the final saved model) in the
NeoBERT intent and NER model directories against their respective
strict holdout sets.

Saves a detailed report to:  src/checkpoint_evaluation_report_neobert.txt

Usage:
    python evaluate_checkpoints_neobert.py
"""
...
# ── Paths ──────────────────────────────────────────────────────────────────
INTENT_PATH     = "./neobert_intent_model"
NER_PATH        = "./neobert_ner_model"
...
REPORT_PATH     = "./checkpoint_evaluation_report_neobert.txt"
...
def evaluate_intent_checkpoint(
    ...
    model = AutoModelForSequenceClassification.from_pretrained(ckpt_path, trust_remote_code=True)
...
def evaluate_ner_checkpoint(
    ...
    model = AutoModelForTokenClassification.from_pretrained(ckpt_path, trust_remote_code=True)
    model.eval().to(DEVICE)
...
def main():
    ...
    lines = [
        SEP,
        "  NEOBERT CHECKPOINT EVALUATION REPORT",
    ...
    intent_tok   = AutoTokenizer.from_pretrained(INTENT_PATH, trust_remote_code=True)
    ...
    ner_tok   = AutoTokenizer.from_pretrained(NER_PATH, trust_remote_code=True)
    ...
    lines += [
        ...
        "",
        "  INTENT MODEL",
        "    Architecture    : NeoBERTForSequenceClassification",
        ...
        "",
        "  NER MODEL",
        "    Architecture    : NeoBERTForTokenClassification",
        ...
        SEP,
        """
  INTENT MODEL  (NeoBERTForSequenceClassification)
  ─────────────────────────────────────────────────────────────────────────────
  ...
  NER MODEL  (NeoBERTForTokenClassification)
  ─────────────────────────────────────────────────────────────────────────────
