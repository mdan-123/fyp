...
NLU_TESTS = [
    ...
    # --- Edge cases that should still go to NLU (10) ---
    ("NLU", "Meeting tomorrow 3pm",                                                     "CREATE_EVENT - terse"),
...
LLM_TESTS = [
    # --- Multi-step / compound scheduling (20) ---
    ("LLM", "Cancel the standup today and instead set up a sync with the engineering team for tomorrow morning.",   "multi-step"),
    ("LLM", "Move my 2pm meeting to Thursday and also remind me an hour before",                                    "multi-step"),
    ("LLM", "Delete the project review and schedule a new one next week with the full team",                        "multi-step"),
    ("LLM", "Reschedule all my afternoon meetings today to tomorrow morning",                                       "multi-step"),
    ("LLM", "Cancel the standup and add a task to send the meeting notes to the team",                              "multi-step"),
    ("LLM", "Book a meeting with Sarah for Friday and set a reminder 30 minutes before and add prep slides to tasks","multi-step"),
    ("LLM", "Move the interview to next week and update my availability block as well",                             "multi-step"),
    ("LLM", "Delete my gym session Saturday and instead schedule a study block for 3 hours",                       "multi-step"),
    ("LLM", "Reschedule the dentist to next Thursday and remind me the day before",                                 "multi-step"),
    ("LLM", "Add a team meeting on Monday and then block my calendar for the rest of the day",                      "multi-step"),
    ("LLM", "Cancel the workshop and create a task to watch the recording instead",                                 "multi-step"),
    ("LLM", "Move the strategy session to Wednesday and also invite the marketing team",                            "multi-step"),
    ("LLM", "Reschedule the lecture and update the associated task deadlines",                                      "multi-step"),
    ("LLM", "Delete the 9am standup and create a new one at 10am with a 30 minute reminder",                       "multi-step"),
    ("LLM", "Cancel my Friday afternoon and reschedule everything to next week",                                    "multi-step"),
    ("LLM", "Add a meeting with the client tomorrow and block two hours before to prepare",                         "multi-step"),
    ("LLM", "Move the project review to next month and set it as a recurring monthly event",                        "multi-step"),
    ("LLM", "Create a task for the report and schedule a 2 hour writing block tomorrow morning",                    "multi-step"),
    ("LLM", "Delete the Friday sync and instead add a task to send a written update by end of day",                "multi-step"),
    ("LLM", "Reschedule my morning meetings and find me a free slot in the afternoon for deep work",                "multi-step"),
    ...
    # --- Conditionals / vague / ambiguous (15) ---
    ...
    # --- Gibberish / incomplete / degenerate inputs (10) ---
...
# ================================================================
def run_tests():
    print("\nInitialising NLU engine...")
    nlu_engine     = SchedulerNLU(INTENT_PATH, NER_PATH, gemini_api_key=None)
    routing_engine = MultiSignalRouter(nlu_engine, CENTROIDS_PATH)

    all_tests   = NLU_TESTS + LLM_TESTS
    results     = {"correct": 0, "wrong": 0, "failures": []}
    nlu_correct = 0
    llm_correct = 0
    nlu_total   = len(NLU_TESTS)
    llm_total   = len(LLM_TESTS)

    print(f"\nRunning {len(all_tests)} tests ({nlu_total} NLU, {llm_total} LLM)...\n")
    print("=" * 70)

    for i, (expected, text, tag) in enumerate(all_tests, 1):
        start  = time.time()
        result = routing_engine.evaluate(text)
        elapsed = (time.time() - start) * 1000

        decision = result.get("decision", "NLU")
        ...
        if correct:
            ...
            results["failures"].append({
                ...
                "text":      text,
                "tag":       tag,
                "reason":    result.get("reason", result.get("intents", "N/A")),
                "latency_ms": round(elapsed, 1)
            })

        # Concise output — full detail only on failures
        if correct:
            print(f"  {status} [{i:03d}] [{tag}] ({elapsed:.0f}ms)")
        else:
            print(f"  {status} [{i:03d}] Expected {expected}, got {decision} | '{text[:60]}...' | {result.get('reason', '')}")
    ...
    print(f"  NLU:      {nlu_correct}/{nlu_total} correct ({(nlu_correct/nlu_total)*100:.1f}%)")
    print(f"  LLM:      {llm_correct}/{llm_total} correct ({(llm_correct/llm_total)*100:.1f}%)")
    print("=" * 70)
    ...
    if results["failures"]:
        ...
        for f in results["failures"]:
            print(f"  [{f['test_num']:03d}] Expected {f['expected']} | Got {f['got']}")
            print(f"        Tag:    {f['tag']}")
            print(f"        Text:   {f['text']}")
            print(f"        Reason: {f['reason']}")
            print(f"        Time:   {f['latency_ms']}ms\n")
